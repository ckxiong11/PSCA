...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        feats = preds[1][0] if isinstance(preds, tuple) else preds[0]
        # feats2 = preds[1][1] if isinstance(preds, tuple) else preds[1]
        feats3 = preds[1][1] if isinstance(preds, tuple) else preds[1]
        # print("feats:", type(feats))
        # print("feats2:",type(feats2))
        ...
        # print(f"anchor_points shape: {anchor_points.shape}")  # 预期是 (B, 8000, 2)
        # print(f"pred_distri shape: {pred_distri.shape}")  # 预期是 (B, 8000, 16)（如果 reg_max=4）
        # Targets
        ...
        _, target_bboxes3, target_scores3, fg_mask3, _ = self.assigner(
            # pred_scores.detach().sigmoid() * 0.8 + dfl_conf.unsqueeze(-1) * 0.2,
        ...
        # loss[1] *= 0.75
        # au_loss = self.bce(pred_scores, target_scores.to(dtype)).sum() / target_scores_sum
        # au_loss *= 0.25
        # loss[1] += au_loss
        loss_main = self.bce(pred_scores3, target_scores3.to(dtype)).sum() / target_scores_sum3  # BCE
        loss_main *= self.hyp.dfl
        loss[1] += loss_main
        # Bbox loss
        if fg_mask.sum():
        ...
        #     au_loss1, au_loss2 = self.bbox_loss(
        #         pred_distri2, pred_bboxes2, anchor_points, target_bboxes2, target_scores2, target_scores_sum2, fg_mask
        #     )
        #     # au_loss1 *= 0.25
        #     # au_loss2 *= 0.25
        #     # loss[0] += au_loss1
        ...
        if fg_mask3.sum():
            ...
            loss0_, loss2_ = self.bbox_loss(
                pred_distri3, pred_bboxes3, anchor_points, target_bboxes3, target_scores3, target_scores_sum3, fg_mask
            )
            loss0_ *= self.hyp.box
            loss2_ *= self.hyp.dfl
            loss[0] += loss0_
